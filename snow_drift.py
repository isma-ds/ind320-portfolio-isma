...
def compute_yearly_results(df, T, F, theta):
    ...
    for s in seasons:
        ...
        season_end = pd.Timestamp(
            year=s + 1, month=6, day=30, hour=23, minute=59, second=59
        )
        ...
        df_season["Swe_hourly"] = df_season.apply(
            lambda row: row["precipitation (mm)"]
            if row["temperature_2m (°C)"] < 1
            else 0,
            axis=1,
        )
...
def compute_fence_height(Qt, fence_type):
    ...
    if fence_type.lower() == "wyoming":
        ...
        raise ValueError(
            "Unsupported fence type. Choose 'Wyoming', 'Slat-and-wire', or 'Solid'."
        )
...
def main():
    ...
    print(
        yearly_df_disp[["season", "Qt (tonnes/m)", "Control"]].to_string(
            index=False, formatters={"Qt (tonnes/m)": lambda x: f"{x:.1f}"}
        )
    )
